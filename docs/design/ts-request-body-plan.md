...
**Progress:** T1 (eval + toggle), T2 (typed body), and T3 (composition — call saved generators, the `{{ }}` replacement) all shipped + browser-verified — commits `ce4e74d`, `cd01af4` (2026-07-23), and `f95d343` (backend, 2026-07-23) + `6c4cc59` (frontend, 2026-07-24). Details in §5. Next up: T4 (ergonomics & migration), which carries the global TS-default flip + token→call migration (§0 #3) — deferred out of T3, which shipped composition opt-in.
...
> **DIRECTION CHANGE (2026-07-24) — supersedes #3 below.** The user decided to **drop the JSON authoring mode entirely** ("any valid JSON is also valid JavaScript, so it should all be JavaScript"). There is no JSON⇄TS toggle and no JSON escape hatch: **TypeScript is the only body mode**, and **metadata** also becomes JavaScript — a **single editor whose value is one `{ [key: string]: string[] }` object** (gRPC metadata is multi-valued), replacing the key/value grid + metadata `{{ }}`. `{{ }}` is removed everywhere; the only "JSON" left is the protojson wire marshalling (unavoidable — object → proto message). **Phase 1 (hidden-wrapper bare-object editor) is DONE + browser-verified — GO** (single-model + `setHiddenAreas`, persist-wrapped; commit `495e086`), so the Monaco spike crux (§0 #4 / risk #4) is **RETIRED**. New phase sequence: **P1** hidden wrapper (DONE `495e086`) → **P2** make TS the only body mode + migrate bodies (**DONE — browser-verified: no toggle, bare object, existing + new requests invoke `0 OK`, `draftBody` is the canonical module; `migrateBodyToTs` covers empty/`{}`/old-TS/JSON+`{{token}}`→call**) → **P4** (resequenced BEFORE P3) metadata as a single `{[k]:string[]}` JS editor (**DONE — browser-verified: eval + multi-valued headers work — `["Bearer "+"xyz"]`→`authorization: Bearer xyz`, `["read","write"]`→two `x-scope` header values; body editor un-regressed. Proto adds `metadata_script` (Invoke/InvokeStream) + `draft_metadata_script` (Request/UpdateRequest/storage) string fields — Struct fields kept; backend `resolveInvokeMetadata` evals via `RunRequestBody`→Struct→existing `structToMetadata`; frontend `MetadataEditor` (parallel to Editor.tsx, body left byte-identical) + `migrateMetadataToTs`**) → **P3** backend cleanup (**DONE — commit `b09ffc4`; deleted `tokens.go`/`tokens_test.go` (the whole `{{ }}` chain, −607 lines), `resolveInvokeBody` now always-evals (dropped the `lang` param + `lang != TYPESCRIPT` bypass; empty body/stream → new `emptyTSBody = "export default () => ({})"`), removed the dead `BodyLanguage` `UpdateRequest` patch path (`store.RequestPatch.BodyLanguage` + `fs.go` apply/guard + `workspace.go` assign) + JSON-schema `Message.schema` generation (`workspace.go messageSchema`); `loadGenerators` moved verbatim into `invoke.go`. KEPT for back-compat: the proto `body_language` enum/fields, the disk⇄wire `BodyLanguage` bridges + their `convert.go` CRUD round-trip, the `Message.schema` proto field, `descriptor_set`, and the frontend `tokens.ts` + `migrateBodyToTs`/`migrateMetadataToTs` (legacy data normalized on load, so the backend never sees a token/raw-JSON). **UPDATE (2026-07-24, remove-body-language-plan.md):** the `body_language` enum/fields + the `BodyLanguage` disk⇄wire bridges listed here were subsequently removed — nothing ever read the field, protojson's `DiscardUnknown` keeps old on-disk `request.json` files (which may still carry `bodyLanguage`) readable, and per AGENTS.md's pre-release stance no `reserved` markers were added. gazelle side effect: `metadata_test.go` (added P4) was missing from BUILD and had never compiled — now built + passing. Gates: `bazelisk build //ui:ui //service/cmd` ok + `bazelisk test //service/...` 4/4. Browser-verified on `:10001`: body invoke `0 OK` via always-eval, evaluated multi-valued metadata intact + reflected `Get status=ok`, `draftBody` persisted canonical, type-check driven by `descriptor_set`, console clean**) → **P5** typed generator signatures (**DONE — browser-verified; the feature is now COMPLETE**). Each saved generator's `export default` signature is inferred and surfaced in autocomplete instead of `(...args: any[]): any`, in BOTH the body + metadata editors. New `ui/src/features/workspace/generator-libs.ts` centralizes the mechanism: for each emittable generator, register its raw source as an isolated virtual TS module at `file:///grpcview/request/gen/<scope>/<name>.ts` and declare `const <name>: typeof import("./gen/<scope>/<name>").default` in a `declare global` block — so Monaco's own TS worker infers the real params/return (full inference, unlike isolated-declarations, which would emit `any` for un-annotated generators). The `generators` prop was widened from `string[]` (names) to `GeneratorDef[]` (`{name, source}`) through RequestWorkspace→RequestPane→MessageTab/MetadataTab→Editor/MetadataEditor. Invariants carried from T3: `isEmittableName` (simple-ident minus reserved words) gates BOTH module + global emission (a name like `default` is skipped whole, never breaking the globals parse); each generator is its own module so a broken/unresolvable-import source degrades to `any` in isolation; `scope` namespaces body vs metadata URIs so neither editor's dispose yanks the other's libs. Browser-verified on `:10001`: signature help `rep(n: number): number`, `rep("oops")`→"Argument of type 'string' is not assignable to parameter of type 'number' (2345)", inferred-return `bla(): string`, and the metadata editor independently shows the same typed generators. `bazelisk build //ui:ui //service/cmd` green. Implementation ran via sequential **background agents**; the main thread orchestrated + browser-verified + committed per milestone.
...
3. **Rollout: TS is the default; migrate existing.** JSON stays only as an escape-hatch mode. Migration: an existing object body renders **as-is** under the hidden wrapper (a JSON object literal is already a valid TS expression — visually seamless); existing **body** `{{ name(args) }}` tokens rewrite mechanically to `name(args)` calls; **metadata** `{{ }}` stays this pass. *Sequencing (updated — landed opt-in in T3; flip + migration are T4):* composition itself shipped **opt-in in T3** (a per-request TS toggle); the global flip to TS-default + token→call migration were **deferred to T4**, because the seamless as-is rendering above depends on T4's not-yet-built hidden wrapper. Until T4, requests that use `{{ }}` remain JSON; new requests can default to TS from T1 (they have no tokens yet).
...
- **T2 — Typed body (pillar B). ✅ DONE — commit `cd01af4`, 2026-07-23, browser-verified.** Run `protoc-gen-es` in-browser over the reflected descriptor (proven §2.5/§4.4) → editor type injection + error footer. *Verified in-browser (self-reflection binary, `WorkspaceService/Get`):* `bogusField` → TS2353 "does not exist in type `GetRequestJson`"; `workspaceName` → `(property) workspaceName?: string` with proto provenance; `workspaceName: 123` → TS2322 citing the real `service_pb.ts`; a valid body reads "valid GetRequest". JSON mode + `{{ }}` unaffected.
  - *Delta from plan (§4.4 "already available" was wrong):* the reflected FDS is **not** on the client for reflection sources (only descriptor-set uploads round-trip their bytes). Added `Workspace.descriptor_set` (merged/deduped/transitive via `desc.ToFileDescriptorSet`) + `Message.file`, **computed at resolution time and cached alongside services** — NOT in the Get handler (Get is refetched after every mutation, so per-Get reflection would regress it) and NOT persisted to `grpcview.store.v1` (derived). `mergeDescriptorSet` folds incrementally on Add, rebuilds on Remove.
  - *Delta — client generator specifics (measured):* package is **2.12.1**; the import is the deep CJS path `@bufbuild/protoc-gen-es/dist/cjs/src/protoc-gen-es-plugin.js` (named `protocGenEs`, no `exports`/ESM). Feed generated `_pb.ts` **raw** to Monaco (risk #7 lean); `typescript` + `@typescript/vfs` aliased to inert stubs in `vite.config.ts` → singlefile bundle **26 MB vs ~36 MB** (tree-shaking can't drop the CJS `typescript` blob; the alias severs the dead static edge — proven inert on `target=ts`).
  - *Delta — Monaco resolution:* `moduleResolution: NodeJs` (node10, from `monaco-scripts.ts`) ignores the package `exports` map, so the real `@bufbuild/protobuf` files are unreachable → hand-written virtual `.d.ts` stubs (`vendor/bufbuild-stubs.ts`) for `@bufbuild/protobuf{,/wkt,/codegenv2}`. `RequestMessage` symbol resolved by scanning the generator's authoritative `Message<"full.proto.Name">` literals (handles **nested** input messages — `Request.Response` → `Request_ResponseJson` — and `$`-reserved names), with short-name derivation as a fallback.
  - *Deferred (per plan):* the hidden-wrapper "bare object" authoring mode (needs the Monaco two-model position-mapping spike, §0 #4 / §T4) — T2 uses the explicit return-annotated arrow `export default (): RequestMessage => ({ … })`, auto-seeded on the JSON→TS toggle when the body is trivial.
- **T3 — Composition (pillar C). ✅ DONE — commits `f95d343` (backend, 2026-07-23) + `6c4cc59` (frontend, 2026-07-24), browser-verified.** In TS body mode each saved generator the body references is exposed as an ambient global — the `{{ }}` replacement — composed via a synthetic esbuild prelude + generator-resolver plugin riding the existing `compileGenerator`→`runFresh` path. *Verified in-browser (self-reflection binary, `WorkspaceService/Get`):* a saved generator `mkid` autocompletes (`function mkid(...args: any[]): any`), `mkid()` type-checks ("valid GetRequest", 0 errors), and invoking `{ workspaceName: mkid() }` returned `0 OK` with the composed value `"id-42"` (grpc→connect two-hop confirmed in the server log). `tsc --noEmit` + `bazel build //ui:ui` + `//service/cmd` all green; JSON mode + `{{ }}` unaffected.
  - *Delta — backend composition engine:* each referenced generator becomes an ambient global via a synthetic prelude (`import __gen$i from "grpcview:gen/<name>"; globalThis.<name> = __gen$i`) prepended to the body and bundled per-run by a `generatorResolverPlugin` (esbuild). New `service/scripting/compose.go` (`composeGeneratorPrelude` + `compileRequestBody`) + `compose_test.go`; `bundler.go` refactored (`esbuildBundle` + the resolver, plugin ordering **capability→generator→registry** so a generator's own npm/`dayjs` imports still resolve); `profiles.go` `RunRequestBody` (empty-gens → `runGenerator` verbatim, else compile+run); `sourcemap.go`/`engine.go` gained an `authorPreludeLines` offset so error lines remap to the author's source, not the prepended prelude.
  - *Delta — failure isolation:* only the generators the body actually **calls** get bundled, bounded by a call-site scan (`name(`, excluding `.name(`) in `invoke.go`'s `referencedGenerators` — mirroring the `{{token}}` model. An unrelated broken generator (even one whose name collides with an object key/method like `id`/`toString`) can't break a valid body; regression-locked by `service/workspace/body_test.go`.
  - *Delta — frontend (ambient globals):* saved generator names (`workspace.scripts` filtered to `ScriptKind.GENERATOR`, memoized) thread `RequestWorkspace`→`RequestPane`→`MessageTab`→`Editor`, which in TS mode registers them as ambient `declare function <name>(...args: any[]): any` globals (`addExtraLib` at a constant path distinct from the T2 typed-body libs, dispose-before-add on change). Only simple-identifier names minus JS reserved words are emitted — a generator named e.g. `default` would emit an invalid decl that silently kills **all** autocomplete — so such names (and dotted names) degrade gracefully (skipped from autocomplete, still run server-side). Autocomplete/typing only; composition runs on the backend.
  - *Scope (locked) — opt-in; migration deferred to T4:* T3 shipped composition **opt-in only** — TypeScript stays a per-request toggle. The global TS-default flip and the auto-migration of existing `{{ }}`-token bodies into direct calls were **deferred to T4** (the "seamless" migration in §0 #3 depends on T4's not-yet-built hidden wrapper), so T3 did **not** flip the default and did **not** migrate existing bodies.
- **T4 — Ergonomics & migration (optional).** "Bare object" hidden-wrapper mode (needs the Monaco position-mapping spike, §0); **the global TS-default flip + auto-migration of existing `{{ }}`-token bodies into direct generator calls (deferred out of T3 — depends on the hidden wrapper above)**; one-click *convert JSON body → TS*; typed generator signatures; (later) TS metadata + a deprecation path for `{{ }}`.
...
3. **Coexistence vs replacement:** locked (§0 #3) — TS becomes the default and existing bodies migrate; JSON stays as an escape hatch. *Timing (updated):* composition landed opt-in in T3; the global flip + token→call migration are **T4** (they depend on the hidden wrapper).
