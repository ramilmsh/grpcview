...
**Progress:** T1 (eval + toggle) and T2 (typed body) both shipped + browser-verified — commits `ce4e74d` and `cd01af4` (2026-07-23). Details in §5. Next up: T3 (composition — call saved generators, the `{{ }}` replacement), which also carries the global TS-default flip + token→call migration (§0 #3).
