...
export const wrap = (obj: string): string =>
  buildWrapped({ skeleton: META_SKELETON, region: obj });
...
const keyLiteral = (key: string): string =>
  IDENT_RE.test(key) ? key : JSON.stringify(key);
...
export const migrateMetadataToTs = (md?: JsonObject): string => {
  ...
  const lines = entries.map(
    ([key, value]) => `  ${keyLiteral(key)}: [${renderElems(value)}]`,
  );
  return wrap(`{\n${lines.join(",\n")}\n}`);
};
