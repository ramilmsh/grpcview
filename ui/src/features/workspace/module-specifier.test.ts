...
describe("specifierPrefixAt", () => {
  it("matches the four specifier forms", () => {
    ...
    expect(specifierPrefixAt("const m = await import('@/scr")?.typed).toBe(
      "@/scr",
    );
  ...
  it("does not match outside an unterminated specifier", () => {
    expect(
      specifierPrefixAt('import { a } from "#/scripts/ids";'),
    ).toBeUndefined();
...
describe("specifierCompletions", () => {
  it("offers both sigils for an empty specifier", () => {
    expect(
      specifierCompletions("", MODULES, "example", undefined).map(
        (c) => c.insertText,
      ),
    ).toEqual(["#/", "@/"]);
    expect(
  ...
  it("walks the ACTIVE collection for #/, one segment at a time", () => {
    ...
    expect(
      specifierCompletions("#/scripts/", MODULES, "example", undefined),
    ).toEqual([
  ...
  it("replaces only the segment being typed", () => {
    expect(
      specifierCompletions("#/scripts/st", MODULES, "example", undefined),
    ).toEqual([
      { insertText: "stamp", kind: "module", specifier: "#/scripts/stamp" },
    ]);
  });
  ...
  it("treats #/ as workspace-relative for the root collection and for none", () => {
    for (const collection of [".", null, undefined]) {
      expect(
        specifierCompletions("#/", MODULES, collection, undefined).map(
          (c) => c.insertText,
        ),
      ).toEqual(["example/", "other/"]);
    }
  ...
  it("offers nothing for a specifier that is not path-mapped", () => {
    expect(
      specifierCompletions("grpcview:inv", MODULES, "example", undefined),
    ).toEqual([]);
    expect(
      specifierCompletions("./ids", MODULES, "example", undefined),
    ).toEqual([]);
  });
});
...
describe("workspacePathForUri", () => {
  it("maps a registered module URI back to its workspace-relative path", () => {
    ...
    expect(
      workspacePathForUri("file:///grpcview/request/body.ts"),
    ).toBeUndefined();
  });
});
